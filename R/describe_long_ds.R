...
describe_long_dataset <- function(x){
  ...
  ## determine which is missing and fill
  all <- x |> tidyr::expand(PARAMCD, USUBJID)
  x <- x|> dplyr::right_join(all) %>% dplyr::ungroup()
  ...
  summary <- x %>%
    ...
    tidyr::pivot_longer(cols = c(-PARAM, -PARAMCD), names_to = "statistics", values_to = "values")
